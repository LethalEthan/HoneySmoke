...
func CreatePacketReader(Data []byte) *PacketReader {
	...
	pr.Data = Data
	pr.Seeker = 0
	pr.End = int32(len(Data))
	return pr
}

//Seek - Seek through the Data array
func (pr *PacketReader) Seek(offset int32) (int32, error) {
	pr.Seeker += offset
...
func (pr *PacketReader) SeekTo(pos int32) bool {
	if pos > pr.End {
		return false
	}
	pr.Seeker = pos
...
func (pr *PacketReader) CheckForEOFWithSeek(SeekTo int32) bool {
	if pr.Seeker > pr.End {
		return true
	}
...
func (pr *PacketReader) SeekWithEOF(offset int32) (int32, error) {
...
func (pr *PacketReader) ReadVarLong() (int64, error) {
	...
	_, err = pr.SeekWithEOF(int32(NumRead))
	if err != nil {
		return Result, err
	}
	return Result, nil
...
func (pr *PacketReader) ReadByteArray(length int32) ([]byte, error) {
	...
	Data := pr.Data[pr.Seeker : pr.Seeker+int32(length)]
	fmt.Print("Datalen: ", len(Data))
	pr.SeekWithEOF(int32(length))
